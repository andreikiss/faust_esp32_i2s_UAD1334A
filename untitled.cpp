...
Compilation options: -a /usr/local/share/faust/esp32/esp32.cpp -lang cpp -i -ct 1 -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
class esp32audio : public audio {
        ...
        void audioTask()
        {
            while (fRunning) {
                if (INPUTS > 0) {
                    ...
                    size_t bytes_read = 0;
                    //i2s_channel_read(m_i2s_tx_handle, &samples_data_in, AUDIO_MAX_CHAN*sizeof(float)*fBufferSize, &bytes_read, portMAX_DELAY);
                ...
                i2s_channel_write(m_i2s_tx_handle, &samples_data_out, AUDIO_MAX_CHAN*sizeof(float)*fBufferSize, &bytes_written, portMAX_DELAY);
            }
    ...
    public:
        ...
        // #endif
            ...
            m_i2s_chan_cfg.id            = (i2s_port_t)I2S_NUM_0;  // I2S_NUM_AUTO, I2S_NUM_0, I2S_NUM_1
            ...
            m_i2s_chan_cfg.dma_frame_num = 294;                    // I2S frame number in one DMA buffer.
            m_i2s_chan_cfg.auto_clear    = true;                   // i2s will always send zero automatically if no data to send
            //m_i2s_chan_cfg.intr_priority = 5;
            i2s_new_channel(&m_i2s_chan_cfg, &m_i2s_tx_handle, NULL);
            
            m_i2s_std_cfg.slot_cfg                = I2S_STD_PHILIPS_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_24BIT, I2S_SLOT_MODE_STEREO); // Set to enable bit shift in Philips mode
            m_i2s_std_cfg.gpio_cfg.bclk           = GPIO_NUM_33;           // BCLK, Assignment in setPinout()
            m_i2s_std_cfg.gpio_cfg.din            = I2S_GPIO_UNUSED;           // not used
            m_i2s_std_cfg.gpio_cfg.dout           = GPIO_NUM_25;           // DOUT, Assignment in setPinout()
            m_i2s_std_cfg.gpio_cfg.mclk           = I2S_GPIO_UNUSED;           // MCLK, Assignment in setPinout()
            m_i2s_std_cfg.gpio_cfg.ws             = GPIO_NUM_26;           // LRC,  Assignment in setPinout()
            ...
            m_i2s_std_cfg.clk_cfg.sample_rate_hz = uint32_t(fSampleRate);
            m_i2s_std_cfg.clk_cfg.clk_src        = I2S_CLK_SRC_DEFAULT;        // Select PLL_F160M as the default source clock
            m_i2s_std_cfg.clk_cfg.mclk_multiple  = I2S_MCLK_MULTIPLE_768;      // mclk = sample_rate * 256
...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /usr/local/share/faust/esp32/esp32.cpp -lang cpp -i -ct 1 -es 1 -mcd 16 -mdd 1024 -mdy 33 -single -ftz 0");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = 0.00091875f * float(fEntry0);
		float fSlow1 = 0.00091875f * float(fEntry1);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec2[0] = fSlow0 + 0.99908125f * fRec2[1];
			float fTemp0 = std::max<float>(1.1920929e-07f, std::fabs(fRec2[0]));
			float fTemp1 = fRec0[1] + 2.0833333e-05f * fTemp0;
			float fTemp2 = fTemp1 + -1.0f;
			int iTemp3 = fTemp2 < 0.0f;
			fRec0[0] = ((iTemp3) ? fTemp1 : fTemp2);
			float fRec1 = ((iTemp3) ? fTemp1 : fTemp1 + (1.0f - 4.8e+04f / fTemp0) * fTemp2);
			fRec3[0] = fSlow1 + 0.99908125f * fRec3[1];
...
void untitled::setParamValue(const string& path, float value)
{
    fUI->setParamValue(path, value);
}

float untitled::getParamValue(const string& path)
{
